...
char into_minsucule(char c) { //Je transforme les majuscules en minuscules et laisse les minuscules comme elles sont*
if ( c<='Z' && c>='A') { 
    c += 'a'-'A' ;
}
return c ;
...
int ajouterOccurence(Index *index, char *mot, int ligne, int ordre, int phrase){
    ...
    if (courant == NULL) {
        ...
        return 1; }  // On gère le cas où l'arbre est vide
    
    while (courant != NULL ) {// Si le mot existe déjà dans l'indexe on s'arrête quand on est dedans, sinon on s'arrête quand on est sur un feuille
        if (compare(courant->mot, mot)==-1) { //Dans ce cas, notre mot se trouve à droite de l'arbre 
            pred = courant ; 
            courant = courant->droit ; 
        }
        
        else {
            if (compare(courant->mot, mot)==1) { // A l'inverse dans ce cas, c'est que notre mot doit être dans la partie gauche
                pred=courant ; 
                courant = courant->gauche ; 
            }

            else {// Dans ce cas, les deux mots sont identiques, donc inutile de créér unnouveau noeud 
                index->nbMotsTotal ++ ; 
                courant->nbOccurence ++ ; 
    ...
    index->nbMotsTotal ++ ; 
    index->nbMotsDistincts ++ ; 
    if (compare(pred->mot,mot)==1) { // Alors on insère le nouveau noeud à gauche 
        pred->gauche=n;        
    }
    else pred->droit=n;//La cas d'égalité a déjà été traité plus haut 
    return 1 ; 
